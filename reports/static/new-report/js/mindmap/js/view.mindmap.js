(function (root, factory) {
    ...
    assign(Markmap.prototype, {
        ...
        init: function (svg, data, options) {
            ...
            var onlyView = this.onlyView = options.onlyView || false;
            var showCircle = this.showCircle = options.showCircle != undefined ? options.showCircle : true;
            state.height = svg.node().getBoundingClientRect().height;
            state.width = svg.node().getBoundingClientRect().width;
            console.log('ClientRect().height, ClientRect().width;', state.height, state.width);
            this.set(options);
            ...
            if (!onlyView) {
                zoom.scaleExtent(this.state.zoomScale ? this.state.zoomScale : [0.1, 10]).on("zoom", function () {
        ...
        updateZoom: function (translate, scale) {
            ...
            // console.log("translate(" + state.zoomTranslate + ")" + " scale(" + state.zoomScale + ")");
            this.svg.attr("transform", "translate(" + state.zoomTranslate + ")" + " scale(" + state.zoomScale + ")")
            if (this.options && this.options.zoomFunction) {
                this.options.zoomFunction(state.zoomScale)
            }
        },
        set: function (values) {
            if (values.preset) {
        ...
        update: function (source) {
            ...
            if (state.autoFit) {
                ...
                // console.log('d3.x', minX, maxX, 'd3.y',minY, maxY);
                var realHeight = maxX - minX;
                var realWidth = maxY - minY;
                // console.log("realWidth,realHeight ", realHeight, realWidth);
                var avgHeight = Math.min(state.width, realWidth) * realHeight / realWidth;
                if (avgHeight + 60 > state.height) {
                    $(this.svgName).height(avgHeight + 60);
                    state.height = avgHeight + 60;
                }
        ...
        renderers: {
            ...
            basic: function (source, nodes, links) {
                ...
                if (!this.onlyView) {
                    nodeEnter.on("click", this.click.bind(this));
                } else {
                    nodeEnter.attr("class", "markmap-node view-markmap-node")
                }
                ...
                var nodeExit = node.exit().transition()
                    .duration(state.duration)
                    .attr("transform", function (d) {
                        return "translate(" + (source.y + source.y_size - d.y_size) + "," + source.x + ")";
                    })
                    .remove();
