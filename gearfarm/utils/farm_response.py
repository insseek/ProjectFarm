...
def json_response_success(data=None, message=''):
    if not message:
        message = '成功'
    return JsonResponse(
        {'result': True, 'result_code': '0', 'result_text': str(message), "message": str(message), 'data': data},
...
def json_response_bad_request(message=''):
    if not message:
        message = '检查请求参数'
    return JsonResponse({'result': False, 'result_code': '10000', 'result_text': str(message), "message": str(message)},
                        status=400,
                        json_dumps_params={'ensure_ascii': False})
...
def json_response_repeated_request(message=''):
    if not message:
        message = '资源存在，无需重复创建'
    return JsonResponse({'result': False, 'result_code': '10001', 'result_text': str(message), "message": str(message)},
                        status=400,
                        json_dumps_params={'ensure_ascii': False})
...
def json_response_unauthorized(message=''):
    if not message:
        message = '用户登录验证失败'
    return JsonResponse({'result': False, 'result_code': '10100', 'result_text': str(message), "message": str(message)},
                        status=401,
                        json_dumps_params={'ensure_ascii': False})
...
def json_response_invalid_authentication_key(message=''):
    if not message:
        message = 'Authentication Key无效'
    return JsonResponse({'result': False, 'result_code': '10104', 'result_text': str(message), "message": str(message)},
                        status=401,
                        json_dumps_params={'ensure_ascii': False})
...
def json_response_authentication_expired(message=''):
    if not message:
        message = 'Token过期'
    return JsonResponse({'result': False, 'result_code': '10105', 'result_text': str(message), "message": str(message)},
                        status=401,
                        json_dumps_params={'ensure_ascii': False})


def json_response_login_required(message=''):
    if not message:
        message = '用户需要登录'
    return JsonResponse({'result': False, 'result_code': '10101', 'result_text': str(message), "message": str(message)},
                        status=401,
                        json_dumps_params={'ensure_ascii': False})
...
def json_response_permissions_required(permissions=None):
    if not permissions:
        ...
        if isinstance(permissions, str):
            permissions = (permissions,)
        message = '缺失权限：{}'.format('，'.join(permissions))
    return JsonResponse({'result': False, 'result_code': '10300', 'result_text': str(message), "message": str(message)},
                        status=403,
                        json_dumps_params={'ensure_ascii': False})
...
def json_response_invalid_permission_key(message=''):
    if not message:
        message = '权限验证失败'
    return JsonResponse({'result': False, 'result_code': '10301', 'result_text': str(message), "message": str(message)},
                        status=403,
                        json_dumps_params={'ensure_ascii': False})
...
def json_response_not_found(message=''):
    if not message:
        message = '所请求的资源不存在'
    return JsonResponse({'result': False, 'result_code': '10400', 'result_text': str(message), "message": str(message)},
                        status=404,
                        json_dumps_params={'ensure_ascii': False})
...
def json_response_error(message=''):
    if not message:
        message = '系统错误'
    return JsonResponse({'result': False, 'result_code': '10500', 'result_text': str(message), "message": str(message)},
                        status=500,
                        json_dumps_params={'ensure_ascii': False})
