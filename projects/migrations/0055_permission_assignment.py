...
def permission_assignment(apps, schema_editor):
    ...
    try:
        permission = project_permissions.get(codename='gear_view_ongoing_project_contracts')
        csm_group.permissions.add(permission)

        permission = project_permissions.get(codename='gear_view_all_project_contracts')
        finance_group.permissions.add(permission)

        permission = project_permissions.get(codename='gear_view_all_my_responsible_projects_contracts')
...
class Migration(migrations.Migration):
    dependencies = [
        ('projects', '0054_auto_20180314_1712'),
    ]
    operations = [
